...
OUR_WINDOW_HWND = None   # set once Tkinter root is created — see TouchlessApp._build_ui
...
def get_browser_wins():
    ...
    if not WIN32_OK:
        return result
    try:
        kws = ["youtube","netflix","prime","spotify","disney","hotstar","hulu",
               "chrome","firefox","edge","brave","vlc","opera"]
        def cb(hwnd, _):
            if not win32gui.IsWindowVisible(hwnd):
                return
            if hwnd == OUR_WINDOW_HWND:
                return   # apni hi Tkinter window ko kabhi target mat karo
...
def execute(action, meta=None):
    ...
    try:
        ...
        target_wins = get_browser_wins()
        focused = focus_media()
        win_title = target_wins[0][1] if target_wins else None
        ...
        if action == "FULLSCREEN":
            if focused:
                pyautogui.press("f")
                return f"Fullscreen ✓ [{win_title}]"
            else:
                if APP_INSTANCE:
                    APP_INSTANCE.ui(APP_INSTANCE._toggle_own_fullscreen)
                return "Fullscreen ✓ [Touchless TV window]"
        ...
        ALWAYS_SAFE = {"SCREENSHOT", "HOME", "EXIT", "COPY", "PASTE", "ZOOM_IN", "ZOOM_OUT"}

        if action in KB:
            if not focused and action not in ALWAYS_SAFE:
                if win_title is None:
                    # Koi bhi matching media window mili hi nahi
                    return f"{action.replace('_',' ').title()} — koi media window open nahi mili, pehle YouTube/Netflix khulo"
                else:
                    # Window mili thi lekin Windows ne focus switch
                    # nahi hone diya — usually permission/timing issue.
                    return f"{action.replace('_',' ').title()} — '{win_title}' mili par focus nahi ho payi, window pe click karke try karo"
            KB[action](); return action.replace("_"," ").title()+" ✓" + (f" [{win_title}]" if win_title else "")
...
class TouchlessApp:
    def __init__(self):
        ...
        self._poll_frame()
        if not WIN32_OK:
            self._log("⚠ pywin32 missing — 'pip install pywin32' karo, warna FULLSCREEN/BACK/etc. kaam nahi karenge","e")
    ...
    def _build_ui(self):
        ...
        # ── Corner spawn — direct full size ─────────────────
        # Window seedha apni full size mein corner pe khulti hai,
        # chhoti se badi hone wala animation hata diya gaya hai.
        self._final_w, self._final_h = 640, 800   # final UI size
        margin = 16
        self.root.geometry(f"{self._final_w}x{self._final_h}+{margin}+{margin}")
        self._is_fullscreen = False

        # Apni hi window ka hwnd capture karo — taaki focus_media()
        # kabhi galti se humein hi target na kare, aur 'f' jaisi
        # keys hamari apni log/text widget mein type na hon.
        global OUR_WINDOW_HWND
        if WIN32_OK:
            try:
                self.root.update_idletasks()
                OUR_WINDOW_HWND = win32gui.FindWindow(None, "Touchless TV Control")
            except Exception:
                OUR_WINDOW_HWND = None
        ...
        # Escape se hamesha fullscreen se bahar nikal sako — safety net
        self.root.bind("<Escape>", lambda e: self._exit_fullscreen())

    def _enter_fullscreen(self):
        """
        Apni khud ki Touchless TV Control window ko fullscreen karo.
        Yeh use hota hai jab OPEN_HAND gesture aaye lekin koi
        YouTube/Netflix jaisi media window na mili ho — to 'f' key
        kahin random jagah bhejne ke bajaye humari apni window hi
        fullscreen ho jaati hai.
        """
    ...
    def _exit_fullscreen(self):
        ...
        self.root.geometry(f"{self._final_w}x{self._final_h}+16+16")
        self.root.resizable(False, False)

    def _toggle_own_fullscreen(self):
        if self._is_fullscreen:
    ...
    def _gesture_loop(self):
        ...
        while self.gesture_on:
            ...
            if res.multi_hand_landmarks:
                ...
                # Draw landmarks ON the real frame
